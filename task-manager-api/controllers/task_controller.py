...
def list_tasks(page, per_page):
    # joinedload traz task + user + category em uma única query (elimina o N+1).
    pagination = Task.query.options(joinedload(Task.user), joinedload(Task.category)).paginate(
        page=page, per_page=per_page, error_out=False
    )
    return {
        "items": [_serialize_task(t) for t in pagination.items],
